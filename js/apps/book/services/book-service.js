...
const booksKey = 'booksDB'

_createBooks().then(_addReviews)

function query() {
  return storageService.query(booksKey)
}
...
function postBook(book) {
  const newBook = {
    ...
    listPrice: {
      amount: utilService.rand(10, 222),
      currencyCode: 'USD',
      isOnSale: Math.random() > 0.5,
    },
  }
  return storageService.post(booksKey, newBook)
}

function getBookFromGoogle(userQuery) {
  const key = 'AIzaSyDMVNy-mubviJ542qOM6smSWO6w3A0QfOc'
  const url = `https://www.googleapis.com/books/v1/volumes?q=${userQuery}&key=${key}`
  return axios.get(url).then((res) => res.data.items)
}

function removeReview(reviewIdx, book) {
  book.reviews.splice(reviewIdx, 1)
  return storageService.put(booksKey, book)
}
...
function getBookById(id) {
  return storageService.get(booksKey, id)
}

function _addReviews() {
  let books
  query()
    .then((booksFromStorage) => (books = booksFromStorage))
    .then((books) => {
      books.forEach((book) => {
    ...
    .then((books) => utilService.saveToStorage(booksKey, books))
}

function _createBooks() {
  return storageService.query(booksKey).then((books) => {
    ...
    utilService.saveToStorage(booksKey, books)
    return books
  })
}
